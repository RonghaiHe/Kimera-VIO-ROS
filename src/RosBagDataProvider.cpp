...
RosbagDataProvider::RosbagDataProvider(const VioParams& vio_params)
  ...
  if (!uwb_topic_.empty()) {
    uwb_pub_ = nh_.advertise<pose_graph_tools_msgs::UWBFrame>(dis_topic_,
                                                              kQueueSize * 18);
  }
  ...
  for (size_t id = 0; id < num_robots_; id++) {
    for (size_t uid = 0; uid < 3; uid++) {
      ros::param::get("~t_body_uwb" + std::to_string(uid),
                      t_uwb_body_[id][uid]);
    }
  }
...
void RosbagDataProvider::sendUWBFrames() {
  ...
  auto u0_pos = Point3(t_uwb_body_[0][0], t_uwb_body_[0][1], t_uwb_body_[0][2]);
  auto u1_pos = Point3(t_uwb_body_[1][0], t_uwb_body_[1][1], t_uwb_body_[1][2]);
  auto u2_pos = Point3(t_uwb_body_[2][0], t_uwb_body_[2][1], t_uwb_body_[2][2]);
  ...
  for (auto it0 = uwb0_map.begin(); it0 != uwb0_map.end(); ++it0) {
    ...
    auto d10 =
        it1 == uwb1_map.end() ? -1 : it1->second->nodes[robot_id_ * 3 + 0].dis;
    auto d12 =
        it1 == uwb1_map.end() ? -1 : it1->second->nodes[robot_id_ * 3 + 2].dis;
    auto d20 =
        it2 == uwb2_map.end() ? -1 : it2->second->nodes[robot_id_ * 3 + 0].dis;
    auto d21 =
        it2 == uwb2_map.end() ? -1 : it2->second->nodes[robot_id_ * 3 + 1].dis;
    ...
    for (auto dest_id = 0; dest_id < num_robots_; dest_id++) {
      ...
      for (auto uid = 0; uid < 3; uid++) {
        ...
        if (src_u0_uwbs.size() > globle_uid &&
            src_u0_uwbs[globle_uid].dis > 0) {
          // 若当前uwb数据不正常，则跳过
          if (unormal[uid]) {
            continue;
          }
          // 若rx-fx > epsilon3，则跳过
          if (src_u0_uwbs[globle_uid].rx_rssi -
                  src_u0_uwbs[globle_uid].fp_rssi >
              epsilon3) {
            continue;
          }
          ...
          if (it0_prev != it0) {
            ...
            if (src_u0_prev_uwbs.size() > globle_uid &&
                src_u0_prev_uwbs[globle_uid].dis > 0) {
              if (fabs(src_u0_uwbs[globle_uid].dis -
                       src_u0_prev_uwbs[globle_uid].dis) < epsilon) {
                continue;
              }
            }
          }

          effect_uwb[0][uid] = boost::make_shared<nlink_parser::LinktrackNode2>(
        ...
        if (it1 != uwb1_map.end()) {
          ...
          if (src_u1_uwbs.size() > globle_uid &&
              src_u1_uwbs[globle_uid].dis > 0) {
            // 若当前uwb数据不正常，则跳过
            if (unormal[uid]) {
              continue;
            }
            // 若rx-fx > epsilon3，则跳过
            if (src_u1_uwbs[globle_uid].rx_rssi -
                    src_u1_uwbs[globle_uid].fp_rssi >
                epsilon3) {
              continue;
            }
            // 前向判断 若当前dis与前一帧差值小于epsilon，则认为无效
            if (it1_prev != it1) {
              ...
              if (src_u1_prev_uwbs.size() > globle_uid &&
                  src_u1_prev_uwbs[globle_uid].dis > 0) {
                if (fabs(src_u1_uwbs[globle_uid].dis -
                         src_u1_prev_uwbs[globle_uid].dis) < epsilon) {
                  continue;
                }
              }
            }
        ...
        if (it2 != uwb2_map.end()) {
          ...
          if (src_u2_uwbs.size() > globle_uid &&
              src_u2_uwbs[globle_uid].dis > 0) {
            // 若当前uwb数据不正常，则跳过
            if (unormal[uid]) {
              continue;
            }
            // 若rx-fx > epsilon3，则跳过
            if (src_u2_uwbs[globle_uid].rx_rssi -
                    src_u2_uwbs[globle_uid].fp_rssi >
                epsilon3) {
              continue;
            }
            // 前向判断 若当前dis与前一帧差值小于epsilon，则认为无效
            if (it2_prev != it2) {
              ...
              if (src_u2_prev_uwbs.size() > globle_uid &&
                  src_u2_prev_uwbs[globle_uid].dis > 0) {
                if (fabs(src_u2_uwbs[globle_uid].dis -
                         src_u2_prev_uwbs[globle_uid].dis) < epsilon) {
                  continue;
                }
              }
            }
      ...
      for (auto& it : checknum) {
        ...
        if (error_num * 1.0 / check_num < 0.5) {
          valid_num++;
          uwb.distances[src_id * 3 + dest_id] =
              effect_uwb[src_id][dest_id]->dis;
        }
      }
